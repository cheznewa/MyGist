...
while True:
 ...
 if nextval == u:
  ...
  nextval = int(param[0])
 if bool(n):
  coa = format(ord(o[0]),"02x")
 else:
  cob = format(ord(o[0]),"02x")
 cog = format(ord(o[1]),"02x")
 col = coa + cog + cob
 if param[1] == "add":
 ...
 if bool(n):
  ...
  sys.stdout.write("%s" %(chr(int(col[4:6],16))))
 sys.stdout.write("%s" %(chr(int(col[2:4],16))))
 n=(n+1)%2
 if not o:
  break
 u = u + 1